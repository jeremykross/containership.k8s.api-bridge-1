...
class KubernetesApi extends ApiImplementation {
    ...
    enforceConstraintsForApp(app) {
        this.getHosts((err, hosts) => {
            if(err) {
                return console.error('failed to get hosts in enforceConstraints');
            }
    ...
    deleteContainer(appId, containerId, cb) {
        ...
        return this.getApplications((err, apps) => {
            ...
            return this.updateApplication(appId, {count: newCount}, cb, false);
        });
    }

    updateApplication(appId, csAppDesc, cb, shouldEnforceConstraints) {

        shouldEnforceConstraints = _.isUndefined(shouldEnforceConstraints) ? true : shouldEnforceConstraints;
        ...
        return this.getApplications((err, apps) => {
            ...
            const newApp = _.isNil(_.get(csAppDesc, 'tags.constraints'))
                ? _.merge(existingApp, csAppDesc, initializedConstraints)
                : _.merge(existingApp, csAppDesc);

            const k8sDesc =
                Translator.csApplicationToK8SReplicationController(newApp);
            ...
            return request({
                ...
                if(shouldEnforceConstraints) {
                    console.log(`Enforcing constraints on update.`);
                    this.enforceConstraintsForApp(newApp);
                }
