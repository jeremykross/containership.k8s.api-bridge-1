...
class KubernetesApi extends ApiImplementation {
    ...
    static k8sServiceDescriptionFor(appName, ports, discoveryPorts, clusterIP, isK8SManaged) {
        isK8SManaged = isK8SManaged || false;
        ...
        return{
            ...
            spec: {
                type: 'NodePort',
                clusterIP: clusterIP ? clusterIP : undefined,
                ports: _.map(ports, (port, idx) => {
                    return _.merge({
                        protocol: port.protocol || 'TCP',
                        port: port.containerPort || port.port,
                        targetPort: port.targetPort || port.containerPort || port.port
                    }, discoveryPorts[idx] ?
                        {nodePort: discoveryPorts[idx]} : {});
                }),
                selector: isK8SManaged ? {
                    'k8s-app': appName
                }: {
                    'app': appName
                }
    ...
    createService(appName, k8sPorts, discoveryPorts, clusterIP, cb) {
        const{ handler, errorHandler } = this.sanitizeCallback(cb);

        appName = appName.indexOf('/') === -1 ? `default/${appName}` : appName;
        const [namespace, name] = _.split(appName, '/');

        const service_body = KubernetesApi.k8sServiceDescriptionFor(name, k8sPorts, discoveryPorts);

        
        const req = {
            uri: `${this.k8sApiUrl}/api/v1/namespaces/${namespace}/services`,
            method: 'POST',
            json: true,
            body: service_body
        };
        ...
        return request(req, (err, res) => {
            if(err || res.statusCode !== 201) {
                return errorHandler(`Could not create service: ${service_body.metadata.name} for application: ${appName}`);
            }
    ...
    enforceAllConstraints() {
        this.getHosts((hosts) => {
            this.getApplications((apps) => {
                _.each(apps, (app) => {
                    this.enforceConstraints(hosts, app);
                });
            });
        });
    }
    ...
    //K8S Only.
    createReplicatedPodFromApplications(appName, replicas, apps, isK8SManaged, cb) {

        const args = _.concat(
            _.map(apps, Translator.csApplicationToK8SPodSpec),
            (dest, src) => {
                if(_.isArray(dest)) {
        ...
        // Will create a pod with all of the containers specified in the applications array.
        // Resorces that belong to the pod, rather than container in k8s will take
        // a latter precidence.
        const podSpec = _.mergeWith.apply(null, args);

        const appLabel = (name) => isK8sManaged ? { 'k8s-app': name } : { 'app': name };
        ...
        request({
            uri: `${this.k8sApiUrl}/api/v1/namespaces/${namespace}/replicationcontrollers`,
            method: 'POST',
            ...
            if(err || res.statusCode !== 200) {
                return cb('Failed to update application');
            }
    ...
    createApplication(csAppDesc, cb) {
        ...
        return request({
            ...
            if(_.size(ports) === 0) {
                ...
                const appName = _.get(k8sAppDesc, ['metadata', 'name']);
                this.createService(appName, ports, [], null, (err) => {
                    if(err) {
                        return errorHandler(err);
                    }
