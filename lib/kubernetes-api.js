...
class KubernetesApi extends ApiImplementation {
        ...
        return this.getApplications((err, apps) => {
            ...
            const tags = 
                _.flow(
                    ...
                    _.partial(_.mapKeys, _, (v, k) => `tags.${k}`))(csApp);

            const k8sDesc =
                _.flow(
                    Translator.csApplicationToK8SReplicationController,
                    _.partial(_.set, _, 'spec.template.spec.nodeSelector', tags))(csApp);
    ...
    alterHostTags(host, tags, cb) {
        const req = {
            uri: `${this.k8sApiUrl}/api/v1/nodes/${host}`,
            method: 'PATCH',
            json: true,
            headers: {
                'Content-Type': 'application/merge-patch+json'
            },
            body: _.set({}, 'metadata.labels', _.reduce(tags, (tags, v, k) => {
                tags[`tags.${k}`] = v;
        ...
        request(req, (err, res) => {
            if(err) {
                return console.error(`There was an err ${err} in K8SApi::alterHostTags.`);
                cb(err);
            }
    ...
    addHostTags(host, tags, cb) { this.alterHostTags(host, tags, cb); }

    removeHostTags(host, tagKeys, cb) {
        this.alterHostTags(host, _.reduce(tagKeys, (acc, k) => _.set(acc, k, null), {}), cb);
    }
