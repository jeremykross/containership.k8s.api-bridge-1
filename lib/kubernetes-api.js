...
class KubernetesApi extends ApiImplementation {
    ...
    static k8sServiceDescriptionFor(app_name, ports, discovery_ports, cluster_ip, is_k8s_managed) {
        return {
            kind: 'Service',
            metadata: {
            ...
            spec: {
                type: 'NodePort',
                clusterIP: !_.isEmpty(cluster_ip) ? cluster_ip : undefined,
                ports: _.map(ports, (port, idx) => {
                    return _.merge({
                        protocol: port.protocol || 'TCP',
                        port: parseInt(port.containerPort || port.port),
                        targetPort: parseInt(port.targetPort || port.containerPort || port.port)
                    }, discovery_ports[idx] ?
                        {nodePort: discovery_ports[idx]} : {});
                }),
                selector: is_k8s_managed ? {
                    'k8s-app': app_name
                } : {
                    app: app_name
                }
    ...
    createService(app_name, k8s_ports, discovery_ports, cluster_ip, is_k8s_managed, cb) {
        //const service_body = KubernetesApi.k8sServiceDescriptionFor(k8s_app_desc, k8s_ports, discovery_ports);

        app_name = app_name.indexOf('/') === -1 ? `default/${app_name}` : app_name;

        const [ namespace, name ] = _.split(app_name, '/');

        const service_body = KubernetesApi.k8sServiceDescriptionFor(name, k8s_ports, discovery_ports);

        const req = {
            uri: `${this.k8s_api_url}/api/v1/namespaces/${namespace}/services`,
            method: 'POST',
            json: true,
            body: service_body
        };
    ...
    createApplication(app_desc, cb) {
        ...
        this._executePreDeployMiddleware(app_desc, (err, app_desc) => {
            ...
            return request({
                ...
                return this.createService(app_name, ports, [], null, false, (err) => {
                    if(err) {
                        return cb(err);
                    }
    ...
    createReplicatedPodFromApplications(app_name, replicas, apps, is_k8s_managed, cb) {

        // Creates a k8s pod spec for each provied CS application.
        const k8s_pods = _.map(apps, Translator.csApplicationToK8SPodSpec);

        // Generates a PodSpec with a ContainerSpec for each cs application.
        const pod_spec = _.mergeWith.apply(null, _.concat(k8s_pods,
            (dest, src) => {
                if(_.isArray(dest)) {
        ...
        const app_label = (name) => is_k8s_managed ? { 'k8s-app': name } : { app: name };

        const [ namespace, name ] = _.split(app_name, '/');

        const replication_controller = {
        ...
        request({
            uri: `${this.k8s_api_url}/api/v1/namespaces/${namespace}/replicationcontrollers`,
            method: 'POST',
            body: replication_controller,
            json: true
        }, (err, res) => {
            if(err || res.statusCode !== 200) {
                return cb(Error('Failed to create RC from replicated pods.'));
            }
    ...
    getHosts(cb) {
        return async.parallel({
            ...
            const cs_followers = _.map(k8s_nodes, _.flow(
                Translator.csHostFromK8SNode,
                (cs_node) => _.merge(cs_node, {
                    'mode': 'follower',
                    'praetor': {
                        leader: false,
            ...
            const leader_id = process.argv[6] || os.hostname();
            const leader_name = os.hostname();
            ...
            const cs_leader = {
                ...
                start_time: Date.parse(_.get(k8s_core_service, 'metadata.creationTimestamp')),

                address: {
                    private: this.k8s_api_ip,
                    public: this.k8s_api_ip
                },

                containers: []
            };

            const keyed = _.keyBy(_.concat(cs_leader, cs_followers), 'id');
