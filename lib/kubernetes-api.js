...
class KubernetesApi extends ApiImplementation {
    ...
    updateApplication(appId, csAppDesc, cb, shouldCycleApps) {
        const{ handler, errorHandler } = this.sanitizeCallback(cb);

        shouldCycleApps = shouldCycleApps || false;
        ...
        return this.getApplications((err, apps) => {
            ...
            const csApp = _.merge(existingApp, newApp);

            const k8sDesc =
                Translator.csApplicationToK8SReplicationController(_.merge({}, csApp, {count: shouldCycleApps ? 0 : csApp.count}));
            ...
            async.series([
                ...
                (cb) => {
                    if(shouldCycleApps) {
                        this.waitForPodCount(appId, 0, (err) => {
                            if(err) {
                                console.error('Error cycling pods in updateApplication.');
                                return cb(err);
                            }

                            console.log(`Cycled PODs down, moving up to ${csApp.count}.`);

                            return this.updateApplication(appId, {count: csApp.count}, cb, false);
                        });
                    } else {
                        return cb();
                    }
    ...
    waitForPodCount(applicationName, desiredCount, cb, timeout, checkInterval) {
        timeout = timeout || 60000;
        checkInterval = checkInterval || 4000;
        ...
        function checkCount() {
            return request(`${self.k8sApiUrl}/api/v1/namespaces/default/replicationcontrollers/${applicationName}`,
                (err, res) => {

                    if(err || res.statusCode !== 200) {
                        console.error(`There was a problem checking podcount for ${applicationName}.`);
                        cb(err);
                    }
                    ...
                    if(podCount !== desiredCount) {
                        console.log(`Have ${podCount} remaining pods.`);
                        if(Date.now() - startTime < timeout) {
                            console.log('Retrying.');
                            return setTimeout(checkCount, checkInterval);
                        } else {
                            return cb(Error(`Timeout waiting for podcount (${podCount}) to match ${desiredCount}.`));
                        }
                    } else {
                        cb();
                    }
    ...
    deleteApplication(name, cb) {
        async.series([
            (cb) => {
                return this.updateApplication(name, { tags: { constraints: {} }, count: 0 }, cb);
            },
            (cb) => {
                return this.waitForPodCount(name, 0, cb);
            }
        ], (err, res) => {
                if(err) {
                    console.error(`There was a problem scaling down pods owned by the RC: #{err}.`)
                    cb(err);
                }
