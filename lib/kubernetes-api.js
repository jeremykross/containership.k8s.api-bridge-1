        ...
        async.parallel({
            ...
            etcd_members: (cb) => {
                ...
                request({
                    ...
                    if(err) {
                        logger.error(`Error determing etcd host for k8s deletion on ${host_id}: ${err}.`);
                        cb(err);
                    }
            ...
            async.waterfall([
                (cb) => {
                    const host = _.get(hosts, host_id);
                    const host_url = _.get(host, ['address', 'public']);

                    const etcd_member_id = _.flow(
                        _.partial(_.filter, _, (m) => {
                            const client_urls = _.get(m, 'clientURLs', []);
                            return _.some(client_urls, (c_url) => _.includes(c_url, host_url));
                        }),
                        _.head,
