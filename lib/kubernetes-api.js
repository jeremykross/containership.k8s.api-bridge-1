...
class KubernetesApi extends ApiImplementation {
    ...
    _applyConstraintsToApplication(app, cb) {
        ...
        this.getHosts((err, hosts) => {
            ...
            const tags_to_list = _.flow(
                _.partial(_.get, _, 'tags', {}),
                _.partial(_.omit, _, 'constraints', 'metadata'),
                flatten,
                _.partial(_.reduce, _, (acc, v, k) => _.concat(acc, `${k}.${v}`), []));

            const application_tags_list = tags_to_list(app);
            const tags_list_for_followers = _.map(followers, tags_to_list);

            // If there aren't any application tags, all followers are fair game, otherwise, match them.
            const potential_followers_count = _.size(application_tags_list) === 0 ? _.size(followers) :
                _.size(_.filter(tags_list_for_followers, (follower_tags_list) => {
                    return _.isEmpty(_.difference(application_tags_list, follower_tags_list));
                }));

            const needed_replicas = per_host > 0 ? potential_followers_count * per_host : running;
